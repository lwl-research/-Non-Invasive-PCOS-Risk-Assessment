...
def plot_pp_bar(df_plot):
    ...
    fig = go.Figure(go.Bar(
        ...
        textfont=dict(color=textcolor[::-1], size=17),
        cliponaxis=False,
        hovertemplate="<b>%{y}</b><br>贡献值：%{x:+.2f} 个百分点<extra></extra>",
    ))

    fig.update_layout(
        height=max(420, 58 * len(labels) + 120),
        margin=dict(l=400, r=120, t=24, b=60),
        font=dict(family="Microsoft YaHei, PingFang SC, Arial", size=21),
        yaxis=dict(
            title="",
            type="category",
